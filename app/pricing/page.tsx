    ...
    <SiteShell>
      <div className="container-wide">
        ...
        <div className="text-center max-w-4xl mx-auto mb-12">
          <div className="inline-block bg-[#1F7D53]/10 text-[#1F7D53] px-4 py-2 rounded-full text-sm font-medium mb-6">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-20">
          {packages.map((pkg, index) => (
            <FadeUp key={pkg.name} delay={index * 0.1}>
              <GlassCard 
                className={`p-6 lg:p-8 h-full flex flex-col ${pkg.highlighted ? 'border-2 border-[#1F7D53] relative' : ''}`}
              >
                {pkg.highlighted && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#1F7D53] text-white text-sm font-medium rounded-full">
                    ⭐ Most Popular
                  </div>
                )}
                ...
                <div className="text-center mb-6">
                  ...
                  <div className="inline-block bg-[#1F7D53]/10 text-[#1F7D53] px-3 py-1 rounded-full text-sm font-medium mb-4">
                    {pkg.badgeText}
                  </div>
                  ...
                  <div className="mb-2">
                    <span className="text-4xl font-bold text-[#1F7D53]">{pkg.price}</span>
                    <span className="text-text-muted text-sm">{pkg.period}</span>
                  </div>
                  {pkg.firstMonth && (
                    <p className="text-sm text-text-muted mb-2">
                      First month: <span className="text-[#1F7D53] font-semibold">{pkg.firstMonth}</span>
                    </p>
                  )}
                  {pkg.offer && (
                ...
                <div className="border-t border-border-glass pt-4 mb-4">
                  ...
                  <ul className="space-y-1">
                    {pkg.bonuses.map((bonus) => (
                      <li key={bonus} className="text-xs text-[#1F7D53]">
                        {bonus}
                      </li>
                    ))}
                  </ul>
                ...
                <Button
                  ...
                  className={`w-full ${pkg.highlighted ? 'bg-[#1F7D53] hover:bg-[#255F38]' : 'bg-surface-glass border border-border-glass hover:bg-[#1F7D53]/10'}`}
                >
        ...
        <FadeUp className="mb-20">
          <div className="bg-gradient-to-r from-[#1F7D53]/20 to-[#255F38]/20 border border-border-glass rounded-2xl p-8 text-center">
