            ...
            {services.map((category, index) => (
              <div key={category.title} id={category.title.toLowerCase().replace(/\s+/g, '-')} className="grid lg:grid-cols-2 gap-12 items-start">
                  ...
                  <ul className="space-y-3">
                    {category.items.map((item) => (
                      <li key={item.name} className="flex items-start gap-3">
                        <span className="w-2 h-2 rounded-full bg-[#255F38] mt-2 flex-shrink-0" />
                        <div>
                ...
                <div className="bg-surface-glass border border-border-glass rounded-2xl p-8">
                  ...
                  <a
                    href="/free-audit"
                    className="inline-flex items-center gap-2 bg-[#1F7D53] hover:bg-[#255F38] text-white font-semibold px-6 py-3 rounded-lg transition-all duration-300"
                  >
                    Get Started →
                  </a>
                </div>
