        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {packages.map((pkg, index) => (
            <FadeUp key={pkg.name} delay={index * 0.1}>
              <GlassCard 
                className={`p-6 text-center ${pkg.highlighted ? 'border-2 border-red-600 relative' : ''}`}
              >
                {pkg.highlighted && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-red-600 text-black text-xs font-medium rounded-full">
                    Most Popular
                  </div>
                )}
